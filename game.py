...
TITLE = "Jogo Kodland" # Nome da Janela

GAME_STATE = 'MENU'  # Estado inicial do jogo

MUSIC_ON = True  # Música ligada

# Botão de play
BUTTON_WIDTH = 250
BUTTON_HEIGHT = 50
PLAY_BUTTON_RECT = Rect((0, 0), (BUTTON_WIDTH, BUTTON_HEIGHT))

PLAY_BUTTON_RECT.center = (WIDTH / 2, HEIGHT / 3 + 100)
...
create_enemy('bee_walkleft', 20, 4, BEE_WALK_SPEED)
create_enemy('bee_walkleft', 25, 9, BEE_WALK_SPEED)
create_enemy('bee_walkleft', 15, 9, BEE_WALK_SPEED)
create_enemy('bee_walkleft', 5, 9, BEE_WALK_SPEED)
...
# Toca a música de fundo em loop (infinitamente)
# Use o nome do arquivo da música sem a extensão
music.play("background_music")

AUDIO_BUTTON_RECT = Rect((0, 0), (BUTTON_WIDTH, BUTTON_HEIGHT))

AUDIO_BUTTON_RECT.center = (WIDTH / 2, HEIGHT / 3 + 180) # Ajustado para descer mais

# ---------------- FIM INICIALIZAÇÃO DE ÁUDIO -----------------------



# ---------- CRIANDO O BOTÃO DE SAIR DO JOGO ----------

EXIT_BUTTON_RECT = Rect((0, 0), (BUTTON_WIDTH, BUTTON_HEIGHT))

# Posicionando abaixo do botão de áudio
EXIT_BUTTON_RECT.center = (WIDTH / 2, HEIGHT / 3 + 260)

# ---------- FIM CRIAÇÃO DO BOTÃO DE SAIR DO JOGO ----------
...
def on_mouse_down(pos):
    ...
    if GAME_STATE == 'MENU':
        ...
        # Lógica do Botão de JOGAR 
        if PLAY_BUTTON_RECT.collidepoint(pos):
            GAME_STATE = 'PLAYING'
            return
        
        # Lógica do Botão de Áudio 
        if AUDIO_BUTTON_RECT.collidepoint(pos):
            if MUSIC_ON:
                music.pause()
                MUSIC_ON = False
            else:
        ...
        if EXIT_BUTTON_RECT.collidepoint(pos):
            raise SystemExit # Fecha o jogo

# ---------------- FIM FUNÇÃO DE CLIQUE DO MOUSE ----------------------
...
def draw():
    ...
    if GAME_STATE == 'MENU':
        # Desenha o Título
        screen.draw.text("KODLAND", center=(WIDTH / 2, HEIGHT / 3), color="black", fontsize=80)
        
        # Desenha a caixa clicável
        screen.draw.rect(PLAY_BUTTON_RECT, "white")
        
        # Desenha o texto "JOGAR" no centro do Rect
        screen.draw.text("JOGAR", center=PLAY_BUTTON_RECT.center, color="black", fontsize=40)

        screen.draw.rect(AUDIO_BUTTON_RECT, "white") # Use uma cor diferente para diferenciar

        # Botão SAIR
        screen.draw.rect(EXIT_BUTTON_RECT, "red") # Usando vermelho para destacar
        screen.draw.text("SAIR", center=EXIT_BUTTON_RECT.center, color="black", fontsize=40)
        
        # Escolhe o texto com base no estado do áudio
        if MUSIC_ON:
            audio_text = "ÁUDIO: LIGADO"
        else:
            audio_text = "ÁUDIO: DESLIGADO"

        screen.draw.text(audio_text, center=AUDIO_BUTTON_RECT.center, color="black", fontsize=30)
...
# ------------- ESTADO DO JOGO ---------------------------
    ...
    if GAME_STATE == 'MENU':
        return # Sai da função, não executa a lógica do jogo
...
# ---------------- COLISAO COM O OBJETIVO (VITÓRIA) -----------------------

        if hero.colliderect(goal):
            hero.pos = HERO_START_POSITION

# ---------------- FIM COLISAO COM O OBJETIVO (VITÓRIA) -------------------
